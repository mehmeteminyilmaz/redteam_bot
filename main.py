...
class RedTeamBot:
    """
    MITRE ATT&CK Framework tabanli Red Team Bot
    ...
    def discover(self):
        # HEDEF IP'LERINI BURAYA YAZ
        self.ips = [
            # "10.0.0.10",   # Hedef sunucu 1
            # "10.0.0.20",   # Hedef sunucu 2
            # "10.0.0.30",   # Hedef sunucu 3
            # "10.0.0.40",   # Hedef sunucu 4
        ]
        
        # VEYA nmap ile otomatik kesif yap
        # logger.info(f"Ag kesfi: {self.cfg.network_range}")
        # nm = nmap.PortScanner()
        # nm.scan(hosts=self.cfg.network_range, arguments='-sn -T4')
        # self.ips = sorted(nm.all_hosts())
        
        logger.info(f"{len(self.ips)} hedef yuklendi: {self.ips}")
        return self.ips
    ...
    def run(self):
        ...
        self.discover()
        if not self.ips:
            logger.warning("Hedef IP bulunamadi! main.py icindeki discover() fonksiyonuna IP'leri ekle.")
            return
        ...
        for mid, data in dist.items():
            ...
            try:
                ...
                logger.error(f"Saldiri hatasi ({name}): {e}")
            
            if self.cfg.stagger_enabled:
                d = random.uniform(*self.cfg.delay_between_attacks)
                logger.info(f"Sonraki saldiriya kadar: {d:.0f}s")
                time.sleep(d)
    ...
    def _check_mitre_expectations(self, result: dict):
        ...
        expectations = {
            'T1071.001': 'Beklenen: Beacon, InteractiveC2 alarmi',
            'T1071.004': 'Beklenen: DnsTunnel, C2 alarmi',
            'T1046': 'Beklenen: Scan, ServiceDiscovery alarmi',
            'T1095': 'Beklenen: TcpTunnel, IcmpTunnel alarmi',
            'T1021.001': 'Beklenen: Lateral, RdpLateral alarmi',
            'T1021.002': 'Beklenen: Lateral, SmbLateral alarmi',
            'T1041': 'Beklenen: Exfil, StealthExfil alarmi',
            'T1562': 'Beklenen: BASELINE_POISON, DefenseEvasion alarmi',
        }
...
if __name__ == "__main__":
    cfg = BotConfig(
        network_range="192.168.1.0/24",  # Bu artik kullanilmiyor
    )
    RedTeamBot(cfg).run()